...
public class HellspawnZombie extends EntityZombie {
    public HellspawnZombie(Location location){
        super(EntityTypes.ZOMBIE, ((CraftWorld) location.getWorld()).getHandle());

        this.setPosition(location.getX(), location.getY(), location.getZ());
        this.setCustomName(new ChatComponentText(ChatColor.BLACK + "" + ChatColor.BOLD + "Hellspawn"));
        this.setBaby(true);
        ...
        ((LivingEntity) this.getBukkitEntity()).getEquipment().setHelmet(new ItemStack(Material.WITHER_SKELETON_SKULL));
        ((LivingEntity) this.getBukkitEntity()).getEquipment().setItemInMainHand(new ItemStack(Material.MAGMA_CREAM));

        ((LivingEntity) this.getBukkitEntity()).getEquipment().setHelmetDropChance(0.0f);
        ((LivingEntity) this.getBukkitEntity()).getEquipment().setItemInMainHandDropChance(0.2f);
